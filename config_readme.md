...
| Key | TM config defaults | Description |
| ------------- | ------------- | ------------- |
...
| "altitude" | `49` | Sets the altitude in meters to perform air pressure compensation
| "enable_display" | `false` | Set to false if the optional weather cover is used, otherwise set to true
...
| "enable_send_data_to_homemanager" | `true` | Enables sending of mqtt Home Automation Data if set to true, otherwise set to false
...
| "enable_indoor_outdoor_functionality" | `"Outdoor"` | Enables an Indoor and an Outdoor Enviro Monitor in the same household to communicate with each other via mqtt messages or to capture outdoor Luftdaten or Adafruit IO sensor data when set to true, otherwise set to false
...
| "mqtt_broker_name" | `"localhost"` | Sets the host name of the mqtt broker if mqtt messages are used for Home Automation and/or indoor/outdoor functionality. Set to "" if no mqtt messaging is required
